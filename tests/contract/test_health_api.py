...
class TestHealthAPIContracts:
    ...
    def test_ready_response_schema_when_healthy(
        self, 
    ...
    def test_ready_response_schema_when_unhealthy(
        self, 
        api_client: APIClient,
        monkeypatch
    ) -> None:
    ...
    def test_health_check_result_latency_is_non_negative(
        self, 
        api_client: APIClient
    ) -> None:
        """
        ...
        for check_name, check_result in response.data['checks'].items():
            assert check_result['latency_ms'] >= 0, (
                f"{check_name} check has negative latency: "
                f"{check_result['latency_ms']}"
            )
